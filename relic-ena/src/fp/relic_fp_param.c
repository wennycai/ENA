	...
	TRY {
		...
		switch (param) {
			...
			case KSS_508:
				...
				bn_set_2b(t1, 12);
				bn_sub(t0, t0, t1);//-x
				int len=bn_bits(t0)+1;
				bn_neg(t1, t1);//x
				bn_neg(t0,t0);//x
				bn_copy(&(core_get()->par),t0);
		...
		int8_t *ss=RLC_ALLOCA(int8_t,len);
		core_get()->par_len = 0;
		bn_rec_naf(ss, &len, &(core_get()->par), 2);
		/* Fix corner case to avoid problems with sparse representation. */
		if (ss[0] == -1) {
			ss[0] = 1;
			ss[1] = -1;
		}
		for (int i = 0; i < len && core_get()->par_len < RLC_TERMS; i++) {
			if (ss[i] > 0) {
				core_get()->par_sps[core_get()->par_len++] = i;
			}
			if (ss[i] < 0) {
				core_get()->par_sps[core_get()->par_len++] = -i;
			}
		}
		if (core_get()->par_len == RLC_TERMS) {
			THROW(ERR_NO_VALID);
		}
				break;
#elif FP_PRIME == 511
