/*==========================================================================*/
/*========                                                   ===============*/
/*         Scalar MUltiplication Based on Elliptic Nets (C&Z)               */
/*========                                                   ===============*/
/*==========================================================================*/
...
static void DoubleAndAdd01(fp_t V1[],fp_t S1[],fp_t P1[],fp_t k[],bn_t m){
	int nb,i;	
	nb = bn_bits(m);
	nb = nb - 1;
...
/*improved double and add alg by Rao et al.*/
static void DoubleAndAdd01_lz(fp_t V1[],fp_t S1[],fp_t P1[],fp_t k[],bn_t m){
	int nb,i;	
	nb = bn_bits(m);
	nb = nb - 1;
	dv_t t[4],T0;
...
static void DoubleAndAdd02(fp_t V1[],fp_t S1[],fp_t P1[],fp_t k[],bn_t m){
	int nb,i;	
	nb = bn_bits(m);
	nb = nb - 1;
...
static void DoubleAndAdd02_lz(fp_t V1[],fp_t S1[],fp_t P1[],fp_t k[],bn_t m){
	int nb,i;	
	nb = bn_bits(m);
	nb = nb - 1;
	dv_t t[5],T0;
...
void scalmul_net_cz(ep_t mP,ep_t P,fp_t A,fp_t B,bn_t m,fp_t theta, int version){
	int i;
	fp_t x_sp,y_sp,xp,yp,V1[8],S1[6],P1[6],k[6];
	fp_t t0,t1,t2;
	bn_t exp;
...
/* fill out the first vector of the block */
	...
	bn_set_dig(exp,8);
	fp_exp(V1[6],theta,exp);
	fp_mul(V1[5],V1[5],V1[6]);
	...
	bn_set_dig(exp,15);
	fp_exp(V1[7],theta,exp);
	fp_mul(V1[6],V1[6],V1[7]);
	/*the orginal W(5,0)*/
	fp_sqr(V1[7],V1[5]);
	fp_mul(V1[7],V1[7],V1[5]);
	fp_sub(V1[7],V1[6],V1[7]);
	...
	switch(version){
		case 1:
			DoubleAndAdd01(V1,S1,P1,k,m);
			break;
		case 2:
			DoubleAndAdd01_lz(V1,S1,P1,k,m);
			break;
		case 3:
			DoubleAndAdd02(V1,S1,P1,k,m);
			break;
		case 4:
			DoubleAndAdd02_lz(V1,S1,P1,k,m);
			break;
		default:
			DoubleAndAdd01(V1,S1,P1,k,m);
			break;
...
/*x_sp = xp - theta^(-2)*(V1[2] * V1[4]) / (V1[3] * V1[3])*/
	fp_mul(y_sp,theta,V1[3]);
	fp_sqr(y_sp,y_sp);
	fp_inv(t0,y_sp);
	fp_mul(t1,V1[2],V1[4]);	
	fp_mul(t1,t1,t0);
	fp_sub(x_sp,xp,t1);
	fp_copy(mP->x,x_sp);
/*y_sp=yp*(V1[2]^2*V1[5]-V1[4]^2*V1[1])/V1[3]^3*/
	fp_sqr(y_sp,V1[3]);
	fp_mul(t0,y_sp,V1[3]);
	fp_inv(y_sp,t0);
	fp_sqr(t0,V1[2]);
