...
void pp_add_k18_basic(fp18_t l, ep3_t r, ep3_t q, ep_t p) {
	fp3_t s;
	ep3_t t;
	...
	TRY{
		...
		fp_mul(l[1][2][0], s[1], p->x);
		fp_mul(l[1][1][1], s[2], p->x);
		fp3_mul(s, s, t->x);
		fp_sub(l[0][1][0], t->y[0], s[0]);
		fp_sub(l[0][0][1], t->y[1], s[1]);
		fp_sub(l[0][2][1], t->y[2], s[2]);
		fp_neg(l[0][0][0], p->y);
	}
		CATCH_ANY{
			THROW(ERR_CAUGHT);
...
void pp_add_k18_projc_basic(fp18_t l, ep3_t r, ep3_t q, ep_t p) {
	...
	TRY{
		...
		/* B = t0 = x1 - x2 * z1. */
		fp3_mul(t0, r->z, q->x);
		fp3_sub(t0, r->x, t0);//w2
		/* A = t1 = y1 - y2 * z1. */
		fp3_mul(t1, r->z, q->y);
		fp3_sub(t1, r->y, t1);//w3

		/* D = B^2. */
		fp3_sqr(t2, t0);//vw
		/* G = x1 * D. */
		fp3_mul(r->x, r->x, t2);//v2
		/* E = B^3. */
		fp3_mul(t2, t2, t0);//v2
		/* C = A^2. */
		fp3_sqr(t3, t1);//v2
		...
		/* l10 = - (A * xp). */
		/*fp_mul(l[0][1][0], t1[0], p->x);
		fp_mul(l[0][0][1], t1[1], p->x);
		fp_mul(l[0][2][1], t1[2], p->x);
		fp6_neg(l[0], l[0]);*/
		fp_mul(l[1][0][0], t1[0], p->x);
		fp_mul(l[1][2][0], t1[1], p->x);
		fp_mul(l[1][1][1], t1[2], p->x);
		fp6_neg(l[1], l[1]);

		/* t4 = A * x2. */
		fp3_mul(t4, q->x, t1);//vw2

		/* H = E + F - 2 * G. */
		fp3_sub(t3, t3, r->x);//v2
		fp3_sub(t3, t3, r->x);
		/* y3 = A * (G - H) - y1 * E. */
		fp3_sub(r->x, r->x, t3);//u
		fp3_mul(t1, t1, r->x);
		fp3_mul(r->y, t2, r->y);
		fp3_sub(r->y, t1, r->y);
		/* x3 = B * H. */
		fp3_mul(r->x, t0, t3);//w2v2
		/* z3 = z1 * E. */
		fp3_mul(r->z, r->z, t2);//v2

		/* l11 = J = A * x2 - B * y2. */
		//w3
		fp3_mul(t2, q->y, t0);

		fp_sub(l[0][1][0], t4[0],t2[0]);
		fp_sub(l[0][0][1], t4[1],t2[1]);
		fp_sub(l[0][2][1], t4[2],t2[2]);

		/* l00 = B * yp. */
		fp_mul(l[0][0][0], t0[0], p->y);
		fp_mul(l[0][2][0], t0[1], p->y);
		fp_mul(l[0][1][1], t0[2], p->y);
		r->norm = 0;
	}
		CATCH_ANY{
			THROW(ERR_CAUGHT);
...
void pp_add_k18_projc_lazyr(fp18_t l, ep3_t r, ep3_t q, ep_t p) {
	fp3_t t1, t2, t3, t4;
	dv3_t u1, u2;
	...
	TRY{
		...
		fp3_sub(t4, t4, r->x);//H
		fp3_sub(t4, t4, r->x);
		fp3_sub(r->x, r->x, t4);//y3
#ifdef RLC_FP_ROOM
		fp3_muln_low(u1, t2, r->x);
		fp3_muln_low(u2, t3, r->y);
#else
		fp3_muln_low(u1, t2, r->x);
		fp3_muln_low(u2, t3, r->y);
#endif
		fp3_subc_low(u2, u1, u2);
		fp3_rdcn_low(r->y, u2);

		fp3_mul(r->x, t1, t4);//x3
		fp3_mul(r->z, r->z, t3);//z3

		fp_mul(l[1][0][0], t2[0], p->x);
		fp_mul(l[1][2][0], t2[1], p->x);
		fp_mul(l[1][1][1], t2[2], p->x);
		fp6_neg(l[1], l[1]);

#ifdef RLC_FP_ROOM
		fp3_muln_low(u1, q->x, t2);
		fp3_muln_low(u2, q->y, t1);
#else
		fp3_muln_low(u1, q->x, t2);
		fp3_muln_low(u2, q->y, t1);
#endif
		//fp3_subc_low(u1, u1, u2);
		fp_rdcn_low(l[0][1][0], u1[0]);
		fp_rdcn_low(l[0][0][1], u1[1]);
		fp_rdcn_low(l[0][2][1], u1[2]);

		fp3_rdcn_low(t2, u2);

		fp_sub(l[0][1][0], l[0][1][0], t2[0]);
		fp_sub(l[0][0][1], l[0][0][1], t2[1]);
		fp_sub(l[0][2][1], l[0][2][1], t2[2]);

		fp_mul(l[0][0][0], t1[0], p->y);
		fp_mul(l[0][2][0], t1[1], p->y);
		fp_mul(l[0][1][1], t1[2], p->y);
		r->norm = 0;
	}
		CATCH_ANY{
			THROW(ERR_CAUGHT);
...
//twited ate pairing
void pp_add_lit_k18(fp18_t l, ep_t r, ep_t p, ep3_t q) {
	fp_t t0, t1, t2, t3;
	...
	TRY{
		...
		fp_mul(l[2][0][0], q->x[0], t1);
		fp_mul(l[2][2][0], q->x[1], t1);
		fp_mul(l[2][1][1], q->x[2], t1);
		fp6_neg(l[2], l[2]);

		fp_mul(l[0][1][0], q->y[0], t0);
		fp_mul(l[0][0][1], q->y[1], t0);
		fp_mul(l[0][2][1], q->y[2], t0);
