/*==========================================================================*/
/*===========                                              =================*/
/*                Improved Elliptic Net Alg on KSS curves                   */
/*===========                                              =================*/
/*==========================================================================*/
...
static void precomp(fp18_t F,fp18_t x_q1,fp18_t y_q1,fp18_t xq,fp18_t yq,fp18_t initial_data[],fp18_t V2[],fp18_t V1[]){
	fp18_t alpha,beta,gamma1,S_0,P_0,t0,w2,w13,vt1,vt2;
	fp18_t S1[5],P1[5],tv2[3],tv1[7];
	int i;
...
/*========================================================*/
		...
		fp18_copy(w2,initial_data[5]); // inverse of W(1,1)	
		fp18_copy(w13,initial_data[6]); // inverse of W(1,1)
		fp18_sqr(S_0,tv2[1]);//1S_12
		fp18_mul(P_0,tv2[0],tv2[2]);//1M_12
		for (int j = 0; j <= 4; j++)//6S_12,6M_12
		{
		...
		fp18_mul(vt1,tv1[3],tv1[5]);
		fp18_sqr(vt2,tv1[4]);
		fp18_mul(tv1[6],vt1,w2);
		fp18_mul(t0,vt2,w13);
		fp18_sub(tv1[6],tv1[6],t0);
		fp18_inv(t0,tv1[2]);
		fp18_mul(tv1[6],tv1[6],t0);
...
static void precomp_lz(fp18_t F,fp18_t x_q1,fp18_t y_q1,fp18_t xq,fp18_t yq,fp18_t initial_data[],fp18_t V2[],fp18_t V1[]){
	fp18_t alpha,beta,gamma1,S_0,P_0,w2,w13,vt1,vt2;
	fp18_t S1[5],P1[5],tv2[3],tv1[7];
	int i;
	dv18_t t0,t1;
...
/*========================================================*/
		...
		fp18_mul(vt1,tv1[3],tv1[5]);
		fp18_sqr(vt2,tv1[4]);
		fp18_muln_low(t0,vt1,w2);
		fp18_muln_low(t1,vt2,w13);
		fp18_subc_low(t0,t0,t1);
		fp18_rdc(tv1[6],t0);
		fp18_inv(tv2[0],tv1[2]);
		fp18_mul(tv1[6],tv1[6],tv2[0]);
...
//NAF-form
static void DoubleAndAdd_2(fp18_t initial_data[],fp18_t V2[],fp18_t V1[],bn_t m){
	fp18_t alpha,beta,gamma1,gamma2,delta,w2,w13,vt1,vt2,S_0,P_0,t0;
	fp18_t S1[5],P1[5];
	...
	for(i=nb-1;i>=0;i--){
		int b=naf[i];
			...
			if (b == 0)
			{
				...
				fp18_mul(V1[6],S1[3],P1[4]);
				fp18_mul(t0,S1[4],P1[3]);
				fp18_sub(V1[6],V1[6],t0);
		...
		else if  (b == 1)
		{
				...
				/* V1[6] = (vt1*w2 - vt2 * w13) / V1[2] */
				fp18_mul(vt1,V1[3],V1[5]);
				fp18_sqr(vt2,V1[4]);
				fp18_mul(V1[6],vt1,w2);
				fp18_mul(t0,vt2,w13);
				fp18_sub(V1[6],V1[6],t0);
				fp18_inv(t0,V1[2]);
				fp18_mul(V1[6],V1[6],t0);
		...
		else if(b==-1)
		{
			...
			{
			/* (S[j - 1] * P1[j] - S[j] * P1[j - 1]) */
				fp18_mul(V1[2*j-1],S1[j-1],P1[j]);
				fp18_mul(t0,S1[j],P1[j-1]);
				fp18_sub(V1[2*j-1],V1[2*j-1],t0);

			/* (S[j-1] * P1[j + 1] - S[j + 1] * P1[j-1])*alpha */
				fp18_mul(V1[2*j],S1[j-1],P1[j+1]);		
				fp18_mul(t0,S1[j+1],P1[j-1]);
				fp18_sub(V1[2*j],V1[2*j],t0);
				fp18_mul_dxs(V1[2*j],V1[2*j],alpha);
			}

				/* V1[6] = (vt1*w2 - vt2 * w13) / V1[4] */
				fp18_mul(vt1,V1[3],V1[1]);
				fp18_sqr(vt2,V1[2]);
				fp18_mul(V1[0],vt1,w2);
				fp18_mul(t0,vt2,w13);
				fp18_sub(V1[0],V1[0],t0);
				fp18_inv(t0,V1[4]);
				fp18_mul(V1[0],V1[0],t0);

			/* V2[0] = (S1[0] * P_0 - P1[0] * S_0 )*gamma2*/
			fp18_mul(V2[0],S1[0],P_0);
			fp18_mul(t0,P1[0],S_0);
			fp18_sub(V2[0],V2[0],t0);
			fp18_mul(V2[0],V2[0],gamma2);
			/* V2[1] = (S1[1] * P_0 - P1[1] * S_0) * delta */
			fp18_mul(V2[1],S1[1],P_0);
			fp18_mul(t0,P1[1],S_0);
			fp18_sub(V2[1],V2[1],t0);
			fp18_mul(V2[1],V2[1],delta);
			/* V2[2] = (S1[2] * P_0 - P1[2] * S_0)  */
			fp18_mul(V2[2],S1[2],P_0);
			fp18_mul(t0,P1[2],S_0);
			fp18_sub(V2[2],V2[2],t0);			
		}
	}
...
static void DoubleAndAdd_2_lz(fp18_t initial_data[],fp18_t V2[],fp18_t V1[],bn_t m){
	fp18_t alpha,beta,gamma1,gamma2,delta,S_0,P_0,vt1,vt2,w2,w13;
	fp18_t S1[5],P1[5];
	dv18_t t0,t1;
	int nb,i,len=bn_bits(m) + 1;
	int8_t* naf=RLC_ALLOCA(int8_t, len);
...
/*========================================================*/
		...
		for(i=nb-1;i>=0;i--){
			...
			if (b == 0)
			{
				...
				/* V1[6] = S[3] * P1[4] - S[4] * P1[3] */
				fp18_muln_low(t0,S1[3],P1[4]);
				fp18_muln_low(t1,S1[4],P1[3]);
				fp18_subc_low(t0,t0,t1);
				fp18_rdc(V1[6],t0);
			...
			else if(b==-1)
			{
				...
				fp18_rdc(V1[0],t0);
				fp18_inv(V2[0],V1[4]);
				fp18_mul(V1[0],V1[0],V2[0]);

				/* V2[0] = (S1[0] * P_0 - P1[0] * S_0)* gamma2 */
				fp18_muln_low(t0,S1[0],P_0);
				fp18_muln_low(t1,P1[0],S_0);
				fp18_subc_low(t0,t0,t1);
				fp18_rdc(V2[0],t0);
				fp18_mul(V2[0],V2[0],gamma2);
				/* V2[1] = (S1[1] * P_0 - P1[1] * S_0) * delta */
...
static void DoubleAndAdd(fp18_t initial_data[],fp18_t V2[],fp18_t V1[],bn_t m){
	fp18_t alpha,beta,gamma1,delta,w2,w13,vt1,vt2,S_0,P_0,t0;
	fp18_t S1[5],P1[5];
	int nb,i;
...
/*origin double and add alg with lazy reduction(C&Z)*/
static void DoubleAndAdd_lz(fp18_t initial_data[],fp18_t V2[],fp18_t V1[],bn_t m){
	fp18_t alpha,beta,gamma1,delta,S_0,P_0,vt1,vt2,w2,w13;
	fp18_t S1[5],P1[5];
	dv18_t t0,t1;
	int nb,i;
...
void opt_en_cz_kss(fp18_t F,ep3_t Q,ep_t P,fp_t A,fp_t B,bn_t m,int version){
	int i=0,sign=0;
	fp18_t initial_data[7],V2[3],V1[7],xq,yq,w_n1,w_2n,w_21,w_11,k0,k1,B0,x_q1,y_q1,x_mq,y_mq;
	fp_t xp,yp;
	fp3_t a,d;	
	fp6_t _t0,_t1,_t2;
	bn_t l;
