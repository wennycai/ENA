/*==========================================================================*/
/*==========                                                ================*/
/*          Original Elliptic Net Alg on Twisted KSS curves                 */
/*==========                                                ================*/
/*==========================================================================*/
...
static void precomp(fp18_t F,fp18_t x_q1,fp18_t y_q1,fp3_t xq,fp3_t yq,fp18_t initial_data[],fp18_t V2[],fp3_t V1[]){
	fp18_t beta,gamma1,S_0,P_0,t0,tv2[3];
    	fp3_t alpha,_S1[6],_P1[6],tv1[8],k0;
	int i;
...
static void DoubleAndAdd(fp18_t initial_data[],fp18_t V2[],fp3_t V1[],bn_t m){
	fp18_t beta,gamma1,delta,S_0,P_0,t0;
    	fp3_t alpha,_S1[6],_P1[6];
	int nb,i;
...
/*========================================================*/
	...
	fp18_copy(beta,initial_data[1]);// inverse of W(-1,1)
	fp18_copy(gamma1,initial_data[2]);// inverse of W(-2,1)
	fp18_copy(delta,initial_data[3]); // inverse of W(1,1)
	for(i=nb-1;i>=0;i--){
...
static void DoubleAndAdd_lz(fp18_t initial_data[],fp18_t V2[],fp3_t V1[],bn_t m){
	fp18_t beta,gamma1,delta,S_0,P_0;
	fp3_t alpha,_S1[6],_P1[6];
	dv18_t t0,t1;
	int nb,i;
...
void opt_en_tw_kss(fp18_t F,ep3_t Q,ep_t P,fp3_t A,fp3_t B,bn_t m,int version){
	int i=0,sign=0;
	fp18_t initial_data[6],V2[3],xp,yp,w_n1,w_2n,w_21,w_11,k0,k1,x_q1,y_q1,x_mq,y_mq;
...
/*========================================================*/
		...
		/*the orginal W(5,0)*/
		fp3_sqr(V1[7],V1[5]);
		fp3_mul(V1[7],V1[5],V1[7]);
		fp3_sqr(V1[2],V1[4]);
		fp3_mul(V1[2],V1[2],V1[4]);
		fp3_mul(V1[2],V1[2],V1[6]);
		fp3_sub(V1[7],V1[2],V1[7]);
	...
	//	fp18_inv(initial_data[3],w_21);//w(2,1)^(-1)
		fp18_inv(initial_data[3],w_11);//w(1,1)^(-1)
		fp3_sqr(_t0,V1[4]);
		fp3_mul(_t1,V1[3],V1[5]);
		fp18_zero(initial_data[4]);
		fp18_zero(initial_data[5]);
		
		fp_copy(initial_data[4][0][0][0],_t0[0]);
		fp_copy(initial_data[4][0][2][0],_t0[1]);
		fp_copy(initial_data[4][0][1][1],_t0[2]);
		fp_copy(initial_data[5][0][0][0],_t1[0]);
		fp_copy(initial_data[5][0][2][0],_t1[1]);
		fp_copy(initial_data[5][0][1][1],_t1[2]);
...
/*========================================================*/
	...
	//	fp18_inv(initial_data[3],w_21);//w(2,1)^(-1)
		...
		fp_copy(initial_data[4][0][0][0],_t2[0]);
		fp_copy(initial_data[4][0][2][0],_t2[1]);
		fp_copy(initial_data[4][0][1][1],_t2[2]);
		fp_copy(initial_data[5][0][0][0],_t3[0]);
		fp_copy(initial_data[5][0][2][0],_t3[1]);
		fp_copy(initial_data[5][0][1][1],_t3[2]);
